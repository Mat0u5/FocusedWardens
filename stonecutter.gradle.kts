...
publishMods {
	...
	if (shouldAnnounceDiscord) {
		...
		discord("publishDiscordBody") {
			...
			if (!isDev) {
				content = changelog.map { "# [FocusedWardens version `$version` is out!](https://modrinth.com/mod/focusedwardens/versions)\n" +
					"### Changelog:\n" +
					"```\n$it```\n"  }
			}
			else {
				content = changelog.map { "# [FocusedWardens version `$version` is out!](https://modrinth.com/mod/focusedwardens-dev/versions)\n" +
					"### Changelog:\n" +
					"```\n$it```\n" }
			}
